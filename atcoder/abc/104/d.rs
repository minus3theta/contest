...
fn main() {
  ...
  let n = s.len();
  let mut dp = vec![vec![ModInt::new(0); 4]; n + 1];
  dp[n][3] = 1.into();
  for (i, &c) in s.iter().enumerate().rev() {
    dp[i][3] = dp[i+1][3] * if c == '?' { 3 } else { 1 };
    for j in 0 .. 3 {
      dp[i][j] += match c {
        'A' => {
          dp[i+1][j] + if j == 0 { dp[i+1][j+1] } else { 0.into() }
        }
        'B' => {
          dp[i+1][j] + if j == 1 { dp[i+1][j+1] } else { 0.into() }
        }
        'C' => {
          dp[i+1][j] + if j == 2 { dp[i+1][j+1] } else { 0.into() }
        }
        '?' => {
          dp[i+1][j] * 3 + dp[i+1][j+1]
        }
        _ => unreachable!(),
      }
    }
