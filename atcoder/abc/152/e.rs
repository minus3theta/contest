...
// fn lcm(x: ModInt, y: ModInt) -> ModInt {
//   x * y / gcd(x.x, y.x)
// }
...
fn main() {
  ...
  for &x in &a {
    ...
    while f * f <= v {
      ...
      if fac > 0 {
        let current = factor.entry(f).or_insert(0);
        *current = cmp::max(*current, fac);
      }
      f += 1;
    }
    if v > 1 {
      let current = factor.entry(v).or_insert(0);
      *current = cmp::max(*current, 1);
    }
  }
  ...
  for &x in &a {
    ...
    for (&f, &c) in &factor {
      ...
      b *= ModInt::new(f as i64).pow(c - fac);
    }
    ans += b;
  }
