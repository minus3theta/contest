...
impl Curve {
  ...
  fn intersection(self, other: Curve) -> Vec<Point> {
    use Curve::*;
    match (self, other) {
      (CLine(l1), CLine(l2)) => l1.intersection(l2).into_iter().collect(),
      (CLine(l), CCircle(c)) | (CCircle(c), CLine(l)) => c.intersection_with_line(l),
      (CCircle(c1), CCircle(c2)) => c1.intersection_with_circle(c2),
    }
  }
}
...
fn main() {
  ...
  let h_rad = 2.0 * PI * (h / 12.0 + m / 60.0 / 12.0);
  let m_rad = 2.0 * PI * m / 60.0;
  let h_vec = Vect::from_angle(h_rad) * a;
  let m_vec = Vect::from_angle(m_rad) * b;

  println!("{:.15}", (h_vec - m_vec).abs());
}
