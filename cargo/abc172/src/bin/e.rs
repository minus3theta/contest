...
fn main() {
    ...
    let fact = ModInt::fact(m);
    let inv_fact = ModInt::inv_fact(&fact);
    let perm = |x: usize, y: usize| fact[x] * inv_fact[x - y];
    let comb = |x: usize, y: usize| perm(x, y) * inv_fact[y];
    let mut sum = ModInt::new(0);
    for i in 0..=n {
        sum += comb(n, i) * perm(m - i, n - i) * (-1i64).pow(i as u32);
    }
    println!("{}", sum * perm(m, n));
}
