...
fn main() {
    ...
    let mut dp = vec![vec![ModInt::new(0); s + 1]; s];
    dp[0][0] = 1.into();
    let mut sum_prev = vec![ModInt::new(1); s + 1];
    let mut sum = vec![ModInt::new(0); s + 1];
    for len in 1..s {
        for total in 0..=s {
            if let Some(p) = total.checked_sub(3) {
                dp[len][total] += sum_prev[p];
                sum[total] = sum[total - 1] + sum_prev[p];
            }
        }
        sum_prev = sum;
        sum = vec![ModInt::new(0); s + 1];
    }
