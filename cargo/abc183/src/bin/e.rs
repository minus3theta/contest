...
fn main() {
    ...
    let mut pat = vec![vec![ModInt::new(0); w]; h];
    let mov = [(0, 1), (1, 1), (1, 0)];
    let mut sum = vec![vec![vec![ModInt::new(0); w]; h]; 3];
    pat[0][0] = 1.into();
    for r in 0..h {
        for c in 0..w {
            if fld[r][c] == '#' {
            ...
            if (r, c) != (0, 0) {
                for (i, &(u, v)) in mov.iter().enumerate() {
                    if let (Some(pr), Some(pc)) = (r.checked_sub(u), c.checked_sub(v)) {
                        pat[r][c] += sum[i][pr][pc];
                    }
                }
            }
