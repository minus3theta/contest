...
fn main() {
    ...
    let s = s
        ...
        .collect_vec();
    let mut dp = vec![vec![vec![MInt::new(0); 10]; 1 << 10]; n + 1];
    for (i, &c) in s.iter().enumerate() {
        dp[i + 1][1 << c][c] = 1.into();
        for used in 0..(1 << 10) {
            for last in 0..10 {
                let current = dp[i][used][last];
                dp[i + 1][used][last] += current;
                if c == last {
                    dp[i + 1][used][last] += current;
                } else if (used >> c) & 1 == 0 {
                    dp[i + 1][used | (1 << c)][c] += current;
                }
            }
        }
    }
    ...
    println!(
        "{}",
        dp[n].iter().flat_map(|v| v.iter().cloned()).sum::<MInt>()
    );
}
