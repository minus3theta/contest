...
fn left(below: usize, d: usize) -> usize {
    if d <= below {
        let ub = d.min(below);
        let lb = d.saturating_sub(below + 1).max(1);
        ub.saturating_sub(lb)
    } else {
        (2 * below + 1).saturating_sub(d)
    }
}

fn sub(below: usize, d: usize) -> MInt {
    let lr = MInt::new(2).pow(d.saturating_sub(2) as i64) * left(below, d) as i64;
    if d <= below {
        lr + MInt::new(2).pow(d as i64)
    } else {
        lr
    }
}
...
fn main() {
    ...
    for top in 0..n {
        let top_count = MInt::new(2).pow(top as i64);
        let below = n - top - 1;
        ans += top_count * sub(below, d) * 2;
    }
...
mod tests {
    ...
    #[test]
    fn sub_10_2() {
        assert_eq!(sub(10, 2), 5.into());
    }

    #[test]
    fn sub2() {
        assert_eq!(sub(2, 2), 5.into());
    }

    #[test]
    fn left2() {
        assert_eq!(left(2, 2), 1);
    }

    #[test]
    fn sub1() {
        assert_eq!(sub(1, 2), 1.into());
    }

    #[test]
    fn left1() {
        assert_eq!(left(1, 2), 1);
    }

    #[test]
    fn sub0() {
        assert_eq!(sub(0, 2), 0.into());
    }
