...
fn dfs(adj: &[Vec<usize>], v: usize, visited: &mut [bool]) -> (usize, usize) {
    visited[v] = true;
    let mut size = 1;
    let mut edges = adj[v].len();
    for &u in &adj[v] {
        if visited[u] {
            continue;
        }
        let (s, e) = dfs(adj, u, visited);
        size += s;
        edges += e;
    }
    (size, edges)
...
fn solve(n: usize, es: &[(usize, usize)]) -> MInt {
