...
impl Count {
    ...
    fn step(&self, h: i64, w: i64) -> Self {
        Self {
            xy: self.x + self.y,
            x: self.none + self.xy * (h - 1) + self.x * (h - 2),
            y: self.none + self.xy * (w - 1) + self.y * (w - 2),
            none: self.x * (w - 1) + self.y * (h - 1) + self.none * (h - 2 + w - 2),
        }
    }
}
...
fn main() {
    ...
    match (p.0 == q.0, p.1 == q.1) {
        (true, true) => count.xy += 1,
        (true, false) => count.y += 1,
        (false, true) => count.x += 1,
        (false, false) => count.none += 1,
    }
    for _ in 0..k {
        count = count.step(h, w);
    }
